...
class Jugador:
    def __init__(self,
        ...
        self.__iddle_r = sf.get_surface_from_spritesheet('pygame_2/assets/img/player/iddle/player_idle.png', 5, 1)
        self.__iddle_l = sf.get_surface_from_spritesheet('pygame_2/assets/img/player/iddle/player_idle.png', 5, 1, flip=True)
        self.__walk_r = sf.get_surface_from_spritesheet('pygame_2/assets/img/player/walk/player_walk.png', 6, 1)
        self.__walk_l = sf.get_surface_from_spritesheet('pygame_2/assets/img/player/walk/player_walk.png', 6, 1, flip=True)
        self.__run_r = sf.get_surface_from_spritesheet('pygame_2/assets/img/player/run/player_run.png', 2, 1)
        self.__run_l = sf.get_surface_from_spritesheet('pygame_2/assets/img/player/run/player_run.png', 2, 1, flip=True)
        self.__jump_r = sf.get_surface_from_spritesheet('pygame_2/assets/img/player/jump/player_jump.png', 6, 1)
        self.__jump_l = sf.get_surface_from_spritesheet('pygame_2/assets/img/player/jump/player_jump.png', 6, 1, flip=True)
    ...
    def __set_y_animation_preset(self):
    ...
    def jump (self,jumping =True):
        if jumping and not self.__is_jumping:
            self.__set_y_animation_preset()
        else:
            self.__is_jumping = False
            self.stay()

    def __set_border_limits(self):
        pixels_move = 0
        if self.__move_x > 0 :
    ...
    def do_movement (self,delta_ms):
        ...
        if self.__player_move_time >= self.__frame_rate :
            self.__player_move_time = 0
            self.__rect.x += self.__set_border_limits()
            self.__rect.y += self.__move_y
            #parte relacionado a saltar
            if self.__rect.y < 410:
